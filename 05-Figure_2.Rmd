...
figure_4a<-plotPCA(normTransform(dds_gonad_subset),intgroup=c("time","gene","sox8","sox9"),ntop=Inf,returnData = T) %>% 
  ...
  ggplot(aes(x=PC1,y=PC2,color=group)) + geom_point(size=10) +
  #xlim(c(-50,60)) + ylim(c(-60,30)) +
  theme_bw() +  ggtitle("Gonad Data - Somatic Subset") +
  geom_text(aes(label=ifelse( (sox8+sox9 < 4 | sox8+sox9 > 0) ,as.character(paste0(sox8,sox9)),''),x=PC1,y=PC2),size=7,color="black") +
...
## Figure 4b  - example genes
```{r}

gg_sox_fpkm("Defb36",d=dds_gonad_subset)+theme(axis.text.x=element_blank())+ylab("")
svglite::svglite(paste0("gonad_Defb36_expression_",ts,".svg"),width=5,height=3)
gg_sox_fpkm("Defb36",d=dds_gonad_subset)+theme(axis.text.x=element_blank())+ylab("")
dev.off()

gg_sox_fpkm("Foxl2",d=dds_gonad_subset)+theme(axis.text.x=element_blank())+ylab("")

svglite::svglite(paste0("gonad_Foxl2_expression_",ts,".svg"),width=5,height=3)
gg_sox_fpkm("Foxl2",d=dds_gonad_subset)+theme(axis.text.x=element_blank())+ylab("")
dev.off()

gg_sox_fpkm("Dmrt1",d=dds_gonad_subset)+theme(axis.text.x=element_blank())+ylab("")
...
#Impact on X Chromosome Genes in WT vs Sox9 Null
```{r}
#temp_wt<-subset(res_dds_ovary_wt, abs(log2FoldChange) > 1 & padj< 0.05 & baseMean > 50))
#temp_null<-subset(res_dds_ovary_null, abs(log2FoldChange) > 1 & padj< 0.05 & baseMean > 50))

temp<-dplyr::full_join(tibble::rownames_to_column(as.data.frame(res_dds_ovary_wt),var="Ensembl_ID"),
                 tibble::rownames_to_column(as.data.frame(res_dds_ovary_null),var="Ensembl_ID"),by="Ensembl_ID",suffix=c(".wt",".null")) %>% 
  filter((abs(log2FoldChange.wt) > 1 & padj.wt < 0.05 & baseMean.wt > 50 ) | (abs(log2FoldChange.null) > 1 & padj.null < 0.05 & baseMean.null > 50 ) ) 
temp$symbol<-symbols[match(temp$Ensembl_ID,symbols$gene_id),"gene_name"]
temp$chr<-symbols[match(temp$Ensembl_ID,symbols$gene_id),"chr"]

temp %>% 
  filter(chr=="X" | chr=="5" | chr=="1") %>% 
  ggplot(aes(x=log2FoldChange.wt,y=log2FoldChange.null,color=chr)) + geom_point() +
  xlim(c(-25,15)) + ylim(c(-25,15)) + theme_bw()

#Null GSEA
temp_res1 <-as.data.frame(res_dds_ovary_null[!is.na(res_dds_ovary_null$padj),])
temp_res1$ENSEMBL<-rownames(temp_res1)
temp_res1$CHR<-symbols[match(temp_res1$ENSEMBL,symbols$gene_id),"chr"]
temp_res1<-temp_res1[temp_res1$CHR %in% c(1:19,"X"),]
temp_res1$CHR<-factor(temp_res1$CHR,levels=c(1:19,"X"))
#incm <- do.call( rbind, with(temp, tapply(ENSEMBL, CHR, function(x) CHR == temp[x,"CHR"] ) ))
incm <- do.call( rbind, with(temp_res1, tapply(ENSEMBL, CHR, function(x) CHR == temp_res1[x,"CHR"] ) ))
colnames(incm) <- temp_res1$ENSEMBL
#str(incm)
rowSums(incm)
...
avgLFC = mean( temp_res1$log2FoldChange[isMember] ),
strength = sum( temp_res1$log2FoldChange[isMember] ) / sqrt(sum(isMember)),
pvalue = t.test( temp_res1$log2FoldChange[ isMember ] )$p.value,
CHR = chr) }

(gsea_null<-do.call( rbind, lapply( rownames(incm), testCategory ) ))
sum(gsea$numGenes)

#WT GSEA
temp_res2 <-as.data.frame(res_dds_ovary_wt[!is.na(res_dds_ovary_wt$padj),])
temp_res2$ENSEMBL<-rownames(temp_res2)
temp_res2$CHR<-symbols[match(temp_res2$ENSEMBL,symbols$gene_id),"chr"]
temp_res2<-temp_res2[temp_res2$CHR %in% c(1:19,"X"),]
temp_res2$CHR<-factor(temp_res2$CHR,levels=c(1:19,"X"))
#incm <- do.call( rbind, with(temp, tapply(ENSEMBL, CHR, function(x) CHR == temp[x,"CHR"] ) ))
incm <- do.call( rbind, with(temp_res2, tapply(ENSEMBL, CHR, function(x) CHR == temp_res2[x,"CHR"] ) ))
colnames(incm) <- temp_res2$ENSEMBL
#str(incm)
rowSums(incm)
...
chr = chr,
numGenes = sum( isMember ),
avgLFC = mean( temp_res2$log2FoldChange[isMember] ),
strength = sum( temp_res2$log2FoldChange[isMember] ) / sqrt(sum(isMember)),
pvalue = t.test( temp_res2$log2FoldChange[ isMember ] )$p.value,
CHR = chr) }

(gsea_wt<-do.call( rbind, lapply( rownames(incm), testCategory ) ))
sum(gsea$numGenes)
...
#                       selection=temp+width)
g_sox9_testis<-rtracklayer::import("../data/sox9_testis_idx512_Tue_Jan_28_2020_1800.bigWig",
                       selection=temp+width)
#Fresh ATAC
...
g_atac_sertoli<-rtracklayer::import("../data/sertoli_fresh_ATACseqPE_GW052319_Tue_Jan_28_2020_1807.bigWig",selection=temp+width) #sertoli
g_atac_granulosa<-rtracklayer::import("../data/granulosa_fresh_ATACseqPE_GW052319_Tue_Jan_28_2020_2208.bigWig",selection=temp+width) #granulosa
...
EnrichedHeatmap(mat_atac_granulosa, col = col_fun_atac_granulosa, name = "atac granulosa",column_title = "atac granulsa",column_title_rot=90, axis_name_rot = 90,split=temp$level,pos_line=F,show_heatmap_legend = FALSE,
                top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = col),ylim=c(0,1.2),yaxis = F)) ,use_raster=TRUE) +
EnrichedHeatmap(mat_atac_sertoli, col = col_fun_atac_sertoli, name = "Atac sertoli",column_title = "atac sertoli",column_title_rot=90, axis_name_rot = 90,split=temp$level,pos_line=F,show_heatmap_legend = FALSE,
                top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = col),ylim=c(0,2),yaxis = F)) ,use_raster=TRUE) +
EnrichedHeatmap(mat_8w, col = col_fun_8w, name = "8w OvaryDmrt1",column_title = "8w Ovary Dmrt1",column_title_rot=90, axis_name_rot = 90,split=temp$level,pos_line=F,show_heatmap_legend = FALSE,
                top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = col),ylim=c(0,50),yaxis = F)) ,use_raster=TRUE) +
EnrichedHeatmap(mat_sox9_ovary, col = col_fun_sox9_ovary, name = "8w OvarySox9",column_title = "8w Ovary Sox9",column_title_rot=90, axis_name_rot = 90,split=temp$level,pos_line=F,show_heatmap_legend = FALSE,
                top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = col),ylim=c(0.25,12),yaxis = F)) ,use_raster=TRUE) +
EnrichedHeatmap(mat_dmrt1_sertoli, col = col_fun_dmrt1_sertoli, name = "p7 Sertoli Dmrt1",column_title = "p7 Sertoli Dmrt1",column_title_rot=90, axis_name_rot = 90,split=temp$level,pos_line=F,show_heatmap_legend = FALSE,
                top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = col),ylim=c(0,30),yaxis = F)),use_raster=TRUE ) +
...
EnrichedHeatmap(mat_sox9_testis, col = col_fun_sox9_testis, name = "Adult Testis Sox9",column_title = "Adult Testis Sox9",column_title_rot=90, axis_name_rot = 90,split=temp$level,pos_line=F,show_heatmap_legend = FALSE,
                top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = col),ylim=c(0.02,0.12),yaxis = F)),use_raster=TRUE ) +
