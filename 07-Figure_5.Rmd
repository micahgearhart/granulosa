...
## IGVsnapshot
chr3:117,312,672-117,333,383  #Plppr4
chr11:112,766,063-112,786,763 #Tesco
...
length(temp3<-gran_ctvsox9_sox9[gran_ctvsox9_sox9 %over% vitro_atac[vitro_atac$class=="Sox9 DAR"] ]) 
#length(temp3<-gran_ctvsox9_sox9[gran_ctvsox9_sox9 %over% gran_ctvdmrt1_sox9 | gran_ctvsox9_sox9 %over% gran_ctvdmrt1_dmrt1]) #predicted to be open prior to sox9
...
length(temp4<-gran_ctvdmrt1_dmrt1[gran_ctvdmrt1_dmrt1 %over% vitro_atac[vitro_atac$class=="Dmrt1 DAR"]]) #predicted to be closed prior to sox9
...
length(vitro_atac_combine<-reduce(c(strip_mcols(peak_vitro_atac_cagSox9),strip_mcols(peak_vitro_atac_cagDmrt1))))  #use peak list for cagSox9 and cagDmrt1
length(vitro_atac_combine<-keepSeqlevels(vitro_atac_combine,c(1:19,"X"),pruning.mode="coarse"))  #all female so remove peaks on Y and MT
length(vitro_atac_combine<-suppressWarnings(vitro_atac_combine[!vitro_atac_combine %over% mito_homologs]))
head(names(vitro_atac_combine)<-paste0("vitro_atac_combine_",formatC(1:length(vitro_atac_combine),width=6,format="d",flag="0")))
...
#system.time(vitro_atac_combine_counts<- summarizeOverlaps(vitro_atac_combine,BamFileList(c(fls.atac.sox9.gran,fls.atac.dmrt1.gran),yieldSize = 1e5),mode =Union, ignore.strand=T))
#as.data.frame(apply(assays(vitro_atac_combine_counts)$counts,2,sum))
#save(vitro_atac_combine_counts,file="vitro_atac_combine_counts.rdata")
#load("vitro_atac_combine_reduce_counts.rdata")
load("vitro_atac_combine_counts.rdata")

vitro_dds_combine<-DESeqDataSet(vitro_atac_combine_counts,design = ~1)
mcols(vitro_dds_combine)$basepairs<-width(vitro_atac_combine)
...
stopifnot(all.equal(names(vitro_atac_combine),rownames(f_mean_atac_combine_vitro)))
mcols(vitro_atac_combine)<-f_mean_atac_combine_vitro
#colnames(mcols(vitro_atac))<-gsub("\\.bam","",colnames(mcols(vitro_atac)))

export(vitro_atac_combine,"vitro_atac_combine.bed")
...
stopifnot(all.equal(rownames(vitro_res_combine_dmrt1),names(vitro_atac_combine)))
vitro_atac_combine$baseMean<-vitro_res_combine_dmrt1$baseMean
vitro_atac_combine$cag_dmrt1_lfc<-vitro_res_combine_dmrt1$log2FoldChange
vitro_atac_combine$cag_dmrt1_padj<-vitro_res_combine_dmrt1$padj

stopifnot(all.equal(rownames(vitro_res_combine_sox9),names(vitro_atac_combine)))
vitro_atac_combine$cag_sox9_lfc<-vitro_res_combine_sox9$log2FoldChange
vitro_atac_combine$cag_sox9_padj<-vitro_res_combine_sox9$padj

#View(as.data.frame(vitro_atac_combine_reduce))
#classify
table(vitro_atac_combine$class<-case_when (
  vitro_atac_combine$baseMean > 7 & vitro_atac_combine$cag_dmrt1_padj < 0.05 & vitro_atac_combine$cag_sox9_padj > 0.05 ~ "Dmrt1 DAR",
  vitro_atac_combine$baseMean > 7 & vitro_atac_combine$cag_dmrt1_padj > 0.05 & vitro_atac_combine$cag_sox9_padj < 0.05 ~ "Sox9 DAR",
  vitro_atac_combine$baseMean > 7 & vitro_atac_combine$cag_dmrt1_padj < 0.05 & vitro_atac_combine$cag_sox9_padj < 0.05 ~ "Common DAR",
  vitro_atac_combine$baseMean > 7 ~ "Constitutive",
  TRUE ~ "unclassified"
))



mcols(vitro_atac_combine) %>% as.data.frame() %>% 
 # dplyr::filter(cag_dmrt1_padj < 0.05 | cag_sox9_padj < 0.05) %>% 
  #dplyr::filter((cag_dmrt1.b2 > 3 & control.b2 < 2) | (cag_sox9.b1 > 3 & control.b1 < 2) ) %>% 
  #dplyr::filter(cag_dmrt1.b2 > 4 | control.b2 > 4 |  cag_sox9.b1 > 4 | control.b1 > 4 ) %>% 
  dplyr::filter(class != "unclassified" & class != "Constitutive") %>% 
...
data.frame(table(vitro_atac_combine$class))



vitro_atac_combine_subset<-vitro_atac_combine[vitro_atac_combine$class!="unclassified"]
#View(as.data.frame(vitro_atac_combine_reduce_subset))


#vitro_atac_combine["vitro_atac_combine_reduce104245"]
...
length(temp<-vitro_atac_combine[vitro_atac_combine$class=="Dmrt1 DAR" & vitro_atac_combine$cag_dmrt1_lfc > 1])
length(temp_con<-vitro_atac_combine[vitro_atac_combine$class=="Constitutive" &
                                   vitro_atac_combine$`cag_dmrt1:b2` > 5 & vitro_atac_combine$`cag_dmrt1:b2` < 8 &
                                   vitro_atac_combine$`cag_sox9:b1` > 5 & vitro_atac_combine$`cag_sox9:b1` < 8  &
                                   vitro_atac_combine$`control:b1` > 5 & vitro_atac_combine$`cag_sox9:b1` < 8  &
                                   vitro_atac_combine$`control:b2` > 5 & vitro_atac_combine$`control:b2` < 8])
...
svglite::svglite(paste0("Figure7d_invitro_atac_chip_heatmap_",ts,".svg"),width=8,height=7.5)

EnrichedHeatmap(mat_p5, col = col_fun_p5, name = "chip_dmrt1_ref",column_title = "chip_dmrt1_ref",axis_name_rot = 90,pos_line=F,split=temp$sox_binding,column_title_rot=90,show_heatmap_legend = FALSE,
...
EnrichedHeatmap(mat_p6, col = col_fun_p6, name = "chip_sox9p33",column_title = "chip_sox9p33",axis_name_rot = 90,pos_line=F,split=temp$sox_binding,column_title_rot=90,show_heatmap_legend = FALSE,
                  top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = col),ylim=c(0,5),yaxis = F)) ) +
EnrichedHeatmap(mat_p5, col = col_fun_p5, name = "chip_dmrt1",column_title = "chip_dmrt1",axis_name_rot = 90,pos_line=F,split=temp$sox_binding,column_title_rot=90,show_heatmap_legend = FALSE,
                  top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = col),ylim=c(0,45),yaxis = F)) ) +
