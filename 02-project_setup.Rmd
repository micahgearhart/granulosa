...
c3a<-cbPalette[5]
c3d<-lighten(lighten(lighten(c3a,0.9),0.9),0.9)
c3c<-lighten(lighten(lighten(c3a,1.0),0.9),0.9)
c3b<-lighten(lighten(lighten(c3a,1.0),1.0),0.9)


gg_plotCounts<-function(x="ENSMUSG00000024837",d=dds_invitro,returnData=F) {
...
gg_plotCounts7d<-function(x="ENSMUSG00000024837",d=dds_7d) {
  ...
  #  mutate(time=factor(time,levels=c("fresh","0h","24h","48h","7d"))) %>%
    mutate(log2count=log2(count+1) ) %>%
    ggplot(aes(x=gene, y=log2count,color=gene)) +
    geom_point(position=position_jitter(w=0.1,h=0),size=3) + ggtitle(title) +
    stat_summary(fun.y = mean, fun.ymin = mean, fun.ymax = mean,
                 geom = "crossbar", width = 0.35,size=0.4) +
...
gg_sox<-function(x="ENSMUSG00000024837",d=dds_ovary,colors=rev(colorRampPalette(brewer.pal(11,"RdBu"))(10))) {
  ...
  ggplot(aes(x=group,y=count,color=group)) + geom_point(size=5) +
    stat_summary(fun.y = mean, fun.ymin = mean, fun.ymax = mean,
                 geom = "crossbar", width = 0.35,size=0.4) +
    ...
    scale_color_manual(values=c("magenta",colors[c(10,9,8,7,4,3,2)],"blue")) +
     geom_text(aes(label=ifelse( (sox8+sox9 < 4 & sox8+sox9 > 0) ,as.character(paste0(sox8,sox9)),''),x=group,y=count),colour="black",size=3) +
...
gff3_file<-"../../R_RESOURCES/gencode.vM19.annotation.gff3"
...
save(symbols,transcripts,bl_mm10,file="symbols_M19.rdata")
```
