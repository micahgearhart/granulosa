...
#define transcripts for each set
...
length(unique(sertoli_transcripts$gene_id))
temp_transcripts<-c(sertoli_transcripts,granulosa_transcripts)
temp_transcripts$size<-width(temp_transcripts)
temp_transcripts[grep("Cyp19a1",temp_transcripts$symbol)]
...
temp_transcripts_dedup[grep("Cyp19a1",temp_transcripts_dedup$symbol)]
strand(temp_transcripts_dedup)<-"*"

find_overlapping_peaks<-function(gr,peaklist) {
  hits<-list()
  for (i in seq_along(1:length(gr))) {
    hits[[i]]<-paste(names(peaklist[peaklist %over% (gr[i]+15000)]),collapse=";")
  }
  names(hits)<-gr$gene_id
  hits<-as.data.frame(unlist(hits))
  colnames(hits)<-"Overlapping_Peaks"
  return(hits)
}

peaklist<-c(sertoli_dmrt1,sertoli_sox9,sert_dars,gran_dars)  #dars defined below
x<-find_overlapping_peaks(temp_transcripts_dedup+5000,peaklist)

temp$Overlapping_Peaks<-x[match(temp$Ensembl_GeneID,rownames(x)),"Overlapping_Peaks"]
#View(temp)
...
length(vivo_atac<-reduce(c(peak_p23_atac_XX,peak_p7_atac_XY))) #Just use postnatal data for this figure
...
length(vivo_atac<-vivo_atac[width(vivo_atac) < 10000])
...
#save(vivo_atac_k27ac_counts,file=paste0("vivo_atac_k27ac_counts_",ts,".rdata"))
#load("vivo_atac_k27ac_counts_Mon_Dec_28_2020_1507.rdata")
load("vivo_atac_k27ac_counts_Fri_Jan_08_2021_1056.rdata")
...
# Note - since vivo_atac doesn't have prenatal data in it the lists are shorter.
...
save(vivo_atac,file=paste0("vivo_atac_",ts,".rdata"))
```
...
(fls1a.vivo <- list.files("../data", pattern=glob2rx("*.bam$"),full=TRUE) %>% str_subset("GW052319") %>% str_subset("sertoli_fresh"))
(fls1b.vivo <- list.files("../data", pattern=glob2rx("*.bam$"),full=TRUE) %>% str_subset("101415|112315|102120") )
...
#system.time(atac_counts_postnatal<- summarizeOverlaps(vivo_atac,BamFileList(c(fls1a.vivo,fls1b.vivo),yieldSize = 1e5,asMates = T),mode =Union, ignore.strand=T))
...
#load("atac_counts_postnatal_Thu_Dec_31_2020_1414.rdata")
load("atac_counts_postnatal_2015Fri_Jan_08_2021_1201.rdata")
...
atac_dds<-DESeqDataSet(atac_counts_postnatal_2015,design = ~1)
mcols(atac_dds)$basepairs<-width(vivo_atac)
colData(atac_dds)$sample<-colnames(assays(atac_counts_postnatal_2015)$counts) %>% gsub("\\.R1_trimmed\\.fastq\\.marked\\.bam","",.) %>% 
  gsub("\\.noMT\\.dedup\\.unique","",.) %>% 
  gsub("_GW052319\\.","",.) %>% 
  gsub("futtner_ATAC_","",.) %>% gsub("_R1_trimmed\\.bam","",.) %>% 
  gsub("\\.marked\\.bam","",.) %>%  gsub("\\.bam","",.) 

colData(atac_dds)$source<-"vivo"
#colData(atac_dds)$sex<-factor(c(rep("Female",5),rep("Male",5)))
colData(atac_dds)$sex<-factor(c(rep("Female",2),rep("Male",2)))
colData(atac_dds)$group<-colData(atac_dds)$sex
...
#Move to Figure 5 - Also use 50bp data instead of 2x150 for figure 5
...
(df<-plotPCA(normTransform(vivo_dds),intgroup=c("sex"),returnData=T,ntop=500))
...
scale_color_manual(values = cbPalette[c(1:6)]) + geom_text(x=df$PC1,y=df$PC2,label=paste0(df$name)) 
...
summary(res_vivo_postnatal<-results(vivo_dds,contrast=c("group","Male","Female"),alpha=0.05))
...
vivo_atac$score<- -1*log10(res_vivo_postnatal$padj)


#are they called in the Sert or Granulosa Datasets
table(vivo_atac$sert_call <- vivo_atac %over% peak_p7_atac_XY)
table(vivo_atac$gran_call <- vivo_atac %over% peak_p23_atac_XX)
export(vivo_atac,paste0("vivo_atac_",ts,".bed"))



table(vivo_atac$class<-case_when (
  vivo_atac$sert_call & vivo_atac$padj < 0.05 & vivo_atac$lfc > 1 & vivo_atac$Male > 2.5 ~ "Sertoli", #3.3,1 vs 4.3,2.8
  vivo_atac$gran_call & vivo_atac$padj < 0.05 & vivo_atac$lfc < -1 &  vivo_atac$Female > 2.5 ~ "Granulosa", # vs 2.8,4.3
  abs(vivo_atac$lfc) < log2(1.25) & vivo_atac$sert_call & vivo_atac$gran_call & vivo_atac$Male > 2.5 & vivo_atac$Female > 2.5~ "Constitutive",
  TRUE ~ "unclassified"
))

#Foxl2
vivo_atac[vivo_atac %over% swl("chr9:98,953,608-98,960,126")]

#Sox9
vivo_atac[vivo_atac %over% swl("chr11:112,766,719-112,792,179")]

#Esr2
vivo_atac[vivo_atac %over% swl("chr12:76,112,358-76,137,913")]


mcols(vivo_atac) %>% as.data.frame() %>% 
  dplyr::filter(class != "unclassified") %>% 
  dplyr::filter(!is.na(lfc) & !is.na(padj)) %>% 
...
export(vivo_atac_subset,paste0("vivo_atac_subset_",ts,".bed"))
...
nr5a2_peaks<-vivo_atac_subset[vivo_atac_subset %over% swl("chr1:136803128-137029332")]
nr5a2_peaks[nr5a2_peaks %over% sertoli_dmrt1]
rspo2_peaks<-vivo_atac_subset[vivo_atac_subset %over% swl("chr15:42,953,041-43,193,844")]
rspo2_peaks[rspo2_peaks %over% sertoli_dmrt1]
esr2_peaks<-vivo_atac_subset[vivo_atac_subset %over% swl("chr12:76,095,449-76,209,624")]
esr2_peaks[esr2_peaks %over% sertoli_dmrt1]
...
#export(vivo_atac_subset[vivo_atac_subset$class == "Granulosa"],paste0("vivo_atac_subset_granulosa_",ts,".bed"))
#export(vivo_atac_subset[vivo_atac_subset$class == "Sertoli"],paste0("vivo_atac_subset_Sertoli_",ts,".bed"))

gran_dars<-vivo_atac_subset[vivo_atac_subset$class == "Granulosa"]
seqlevelsStyle(gran_dars)<-"UCSC"
gran_dars<-sortSeqlevels(gran_dars)
gran_dars<-sort(gran_dars)
head(names(gran_dars)<-paste0("granDAR_",formatC(1:length(gran_dars),width=5,format="d",flag="0")))
mcols(gran_dars)<-mcols(gran_dars)[,c("score","lfc","padj","Female","Male")]
export(gran_dars,"gran_dars_peaklist.bed")
write.csv(as.data.frame(gran_dars),"gran_dars_peaklist.csv",row.names = T,quote=F)
seqlevelsStyle(gran_dars)<-"NCBI"

sert_dars<-vivo_atac_subset[vivo_atac_subset$class == "Sertoli"]
seqlevelsStyle(sert_dars)<-"UCSC"
sert_dars<-sortSeqlevels(sert_dars)
sert_dars<-sort(sert_dars)
head(names(sert_dars)<-paste0("sertDAR_",formatC(1:length(sert_dars),width=5,format="d",flag="0")))
mcols(sert_dars)<-mcols(sert_dars)[,c("score","lfc","padj","Female","Male")]
export(sert_dars,"sert_dars_peaklist.bed")
write.csv(as.data.frame(sert_dars),"sert_dars_peaklist.csv",row.names = T,quote=F)
seqlevelsStyle(sert_dars)<-"NCBI"
```


```{r in_vivo_DARs_ChIP_overlap,eval=T}
temp<-vivo_atac_subset[vivo_atac_subset$class == "Sertoli"]
temp[temp %over% swl("chr9:98,953,608-98,960,126")]

#Which ones overlap what sites?
table(vivo_atac_subset$dmrt1<-vivo_atac_subset %over% sertoli_dmrt1)
table(vivo_atac_subset$sox9<-vivo_atac_subset %over% sertoli_sox9)
...
length(vivo_atac_subset[vivo_atac_subset$class=="Sertoli" & vivo_atac_subset$ovary_dmrt1])
...
#Numbers for Text
table(vivo_atac_subset[vivo_atac_subset$class=="Sertoli"]$dmrt1 | vivo_atac_subset[vivo_atac_subset$class=="Sertoli"]$sox9)
mean(vivo_atac_subset[vivo_atac_subset$class=="Sertoli"]$dmrt1 | vivo_atac_subset[vivo_atac_subset$class=="Sertoli"]$sox9)
table(vivo_atac_subset[vivo_atac_subset$class=="Sertoli"]$peak)
...
table(vivo_atac_subset[vivo_atac_subset$class=="Granulosa"]$foxl2 | vivo_atac_subset[vivo_atac_subset$class=="Granulosa"]$esr2)
mean(vivo_atac_subset[vivo_atac_subset$class=="Granulosa"]$foxl2 | vivo_atac_subset[vivo_atac_subset$class=="Granulosa"]$esr2)
table(vivo_atac_subset[vivo_atac_subset$class=="Granulosa"]$peak)
...
#Regions for Figure
 #  chr9:98,947,927-98,961,374 chr11:112,766,547-112,790,986 chr12:76,113,848-76,138,517
 #  chr9:98,947,412-98,967,687 chr11:112,766,547-112,790,986 chr12:76,113,848-76,138,517
#gr<-lapply(c("chr9:9tac_subset %over% gr[[1]]][,c("Female","Male","lfc","padj","class")]
gr<-lapply(c("chr9:98,947,412-98,967,687", "chr11:112,766,547-112,790,986", "chr12:76,113,848-76,138,517"),swl)
vivo_atac_subset[vivo_atac_subset %over% gr[[2]]][,c("Female","Male","lfc","padj","class")]
vivo_atac_subset[vivo_atac_subset %over% gr[[3]]][,c("Female","Male","lfc","padj","class")]
vivo_atac_subset[vivo_atac_subset %over% gr[[1]]][,c("Female","Male","lfc","padj","class")]


#chr13:64,054,124-64,091,964  # HSD17b3
#chr1:136,808,754-136,984,916 # NR5A2
#chr9:54,158,398-54,212,193  #CYP19A1
#chr13:100,479,991-100,568,436 #OCLN

#chr13:64,054,124-64,091,964   chr1:136,808,754-136,984,916  chr9:54,158,398-54,212,193   chr13:100,479,991-100,568,436 
#chrX:53,426,127-53,450,761  chr1:136,808,754-136,984,916  chr9:54,158,398-54,212,193   chr13:100,479,991-100,568,436 w/Etd
#chr1:136,808,754-136,984,916 chr2:148,831,959-148,841,675  chr9:54,158,398-54,212,193   chr13:100,479,991-100,568,436 w/Cst9


#t(log2(f_atac_vivo[roi,]))

g_volcano<-mcols(vivo_atac_subset) %>% as.data.frame() %>% 
  dplyr::filter(!is.na(lfc) & !is.na(padj)) %>% 
...
round(100*table(temp[temp$class=="Sertoli"]$close_to)/as.numeric(table(vivo_atac_subset$class)["Sertoli"]),0)
table(temp[temp$class=="Granulosa"]$close_to)
round(100*table(temp[temp$class=="Granulosa"]$close_to)/as.numeric(table(vivo_atac_subset$class)["Granulosa"]),0)
...
g1<- mcols(temp) %>% as.data.frame() %>% 
  ...
  xlim(xrange) + ylim(yrange) + theme_bw()  + theme(legend.position = "None") +
  xlab("") + ylab("") + ggtitle("short") +
...
g2<- mcols(temp) %>% as.data.frame() %>% 
  ...
  ggplot(aes(x=lfc,y=(-1*log10(padj)),color=close_to)) + geom_point(size=0.7) +
  xlim(xrange) + ylim(yrange) + theme_bw()  + theme(legend.position = "None") +
    xlab("") + ylab("") + ggtitle("long") +
  scale_color_manual(values=c("red","blue")) 

svglite::svglite(paste0("somatic_atac_degs_long_",ts,".svg"),width=3,height=2.5)
#grid.arrange(g2,g1,ncol=1)
g2
dev.off()

svglite::svglite(paste0("somatic_atac_degs_short_",ts,".svg"),width=3,height=2.5)
g1
dev.off()
